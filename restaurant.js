...
class Restaurant {
	constructor (name, latitude, longitude, address, reviews, totalReviews, origin, average, idPlaces) {
		this.name = name;
	...
	addFafastar() {							// Méthode pour afficher des étoiles en fonction de la moyenne
		let fafastar ="";
		//let roundedAverage = this.average;
...
request.onload = function() {
	...
	for (const restaurant of restaurants) {
		...
		let newRestaurant = new Restaurant(restaurant.restaurantName, restaurant.lat, restaurant.long, restaurant.address, restaurant.ratings, restaurant.ratings.length, "originJson", average);
...
// ---=== Récupérer les restaurants de Google Places (via le bouton) ===--- //
...
function addPlaces() {
	...
	service = new google.maps.places.PlacesService(liste2); // Parce qu'il demande un innerHTML
...
function callback(results, status) {
	if (status == google.maps.places.PlacesServiceStatus.OK) {
		...
		for (const result of results) {
			...
			// Créer des nouvelles instances de la classe et les ajouter au tableau
			let newRestaurant = new Restaurant(result.name, result.geometry.viewport.Wa.i, result.geometry.viewport.Sa.i, result.vicinity, newArray, result.user_ratings_total, "originPlaces", Number(result.rating.toFixed(0)), result.place_id);
			restaurantsArray.push(newRestaurant);
		}
